...
```bash
ffmpeg -i your_video.mp4 -vf "fps=328/153" -q:v 2 frames/frame_%04d.jpg
```

### 2. 特征提取与稀疏重建

```bash
python run_LightGlue.py \
    --image_dir ./images/SEU_QiangongBuilding \
    --output_dir ./outputs/SEU_QiangongBuilding_328
```
...
colmap image_undistorter \
    --image_path ./images/SEU_QiangongBuilding \
    --input_path ./outputs/SEU_QiangongBuilding_328 \
    --output_path ./SEU_QiangongBuilding_328_PINHOLE \
    --output_type COLMAP
```
...
```bash
python train.py \
    -s ./SEU_QiangongBuilding_328_PINHOLE \
    -r 4 \
    --iterations 30000
```
...
├── train.py                # 3D Gaussian Splatting 训练（来自官方仓库）
